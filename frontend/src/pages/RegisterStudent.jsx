...
export default function RegisterStudent() {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      showToast("Student account created. Please login.", "success");
  ...
  return (
    <section className="auth-card auth-card-wide">
      <div className="auth-brand"><div className="brand-mark">CL</div><span>Student Registration</span></div>
      ...
      <form className="form two-column" onSubmit={submit}>
        ...
        <label>Full name<input name="full_name" value={form.full_name} onChange={update} placeholder="Ram Student" /></label>
        <label>Email<input type="email" name="email" value={form.email} onChange={update} placeholder="student@example.com" /></label>
        ...
        <label>Date of birth<input type="date" name="date_of_birth" value={form.date_of_birth} onChange={update} /></label>
        <label>Guardian name<input name="guardian_name" value={form.guardian_name} onChange={update} placeholder="Optional" /></label>
        <label>Address<input name="address" value={form.address} onChange={update} placeholder="Optional" /></label>
        <button className="btn btn-primary btn-block span-2" type="submit" disabled={loading}>{loading ? "Creating account..." : "Register as student"}</button>
      </form>
