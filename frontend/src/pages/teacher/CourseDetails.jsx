  ...
  return (
    <div className="page-stack">
      <PageHeader
        ...
        actions={<><Link className="btn btn-secondary" to={`/teacher/courses/${id}/edit`}>Edit course</Link><Link className="btn btn-primary" to="/teacher/batches/create" state={{ courseId: course.id }}>Create classroom</Link></>}
      />
      ...
      <section className="section-card"><div className="section-heading"><h3>Classrooms for this course</h3></div>{batches.length ? <div className="card-grid compact-grid">{batches.map((batch) => <BatchCard key={batch.id} batch={batch} />)}</div> : <EmptyState title="Create your first classroom" />}</section>
      <section className="section-card"><div className="section-heading"><h3>Related live classes</h3></div>{sessions.length ? <div className="card-grid compact-grid">{sessions.map((session) => <SessionCard key={session.id} session={session} />)}</div> : <EmptyState title="No live classes scheduled" />}</section>
    </div>
  );
}
